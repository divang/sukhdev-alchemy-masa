...
if (!supabaseUrl || !supabaseAnonKey || !razorpayKeyId || !razorpayKeySecret) {
  throw new Error("Missing required environment variables for create-order function.")
}
...
function mapRazorpayErrorStatus(statusCode: number) {
...
Deno.serve(async (req) => {
  ...
  if (!gatewayResponse.ok) {
    return jsonResponse(
      ...
      mapRazorpayErrorStatus(gatewayResponse.status)
    )
  }
